    ...
    void processGame(const Value& game, std::vector<ScraperSearchResult>& results)
    {
        ...
        if (game.HasMember("metadatum") && game["metadatum"].IsObject()) {
            ...
            if (metadatum.HasMember("first_release_date") &&
                ...
                result.mdl.set("releasedate", Utils::Time::DateTime(static_cast<time_t>(
                                                  metadatum["first_release_date"].GetInt64())));
            }
            ...
            // RomM doesn't distinguish developer from publisher, so the combined company
            // list is used to populate the developer field.
