...
#
#  ES-DE Frontend
#  macOS_dependencies_build_x86_cross.sh
#
#  Builds the x86 external dependencies in-tree using cross-compilation on an ARM processor.
...
./configure --host=x86_64-apple-darwin --enable-static=yes --enable-shared=no --prefix=$(pwd)/../local_install
make clean
make -j${JOBS}
make install
cd ..
...
./configure --host=x86_64-apple-darwin --disable-libasprintf --with-libiconv-prefix=$(pwd)/../local_install --prefix=$(pwd)/../local_install
make clean
make -j${JOBS}
...
cd icu/icu4c/source
ICU_DATA_FILTER_FILE=icu_filters.json CXXFLAGS="-arch x86_64 -DUCONFIG_NO_COLLATION -DUCONFIG_NO_TRANSLITERATION" ./configure --host=x86_64-apple-darwin --disable-extras --disable-icuio --disable-samples --disable-tests
make clean
make -j${JOBS}
cd lib
...
cat Makefile.osx | sed s/"COMPILERFLAGS_X86_64 = -arch arm64"/"COMPILERFLAGS_X86_64 = -arch x86_64"/ > Makefile.osx_TEMP
cat Makefile.osx_TEMP | sed s/"\$(LIBTOOL) -arch_only arm64 -o \$@ \$(MODULES_X86_64)"/"\$(LIBTOOL) -arch_only x86_64 -o \$@ \$(MODULES_X86_64)"/ > Makefile.osx
rm Makefile.osx_TEMP
...
cd FFmpeg
PKG_CONFIG_PATH=$(pwd)/../local_install/lib/pkgconfig ./configure --arch=x86_64 --cpu=x86_64 --prefix=/usr/local --enable-rpath --install-name-dir=@rpath --disable-doc --disable-lzma --enable-gpl --enable-shared --enable-libvorbis --enable-libopus --enable-libvpx --enable-postproc
make clean
make -j${JOBS}
